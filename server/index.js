...
app.post('/api/track-position', async (req, res) => {
    ...
    console.log(`🎯 Position Tracker for: "${keyword}" | domain: ${domain} | region: ${region}`);

    try {
        const axios = require('axios');
        const cheerio = require('cheerio');

        // Use a generic search URL (can be swapped for Serper.dev or similar API later)
        const searchUrl = `https://www.google.com/search?q=${encodeURIComponent(keyword)}&num=50&gl=${region}`;
        ...
        $('.g').each((i, el) => {
            ...
            if (link && link.startsWith('http')) {
                try {
                    ...
                    searchResults.push({ domain: host, url: link });
                } catch (e) { }
            }
        });
        ...
        const rank = searchResults.findIndex(r => r.domain.includes(cleanDomain)) + 1;
        ...
        res.json({
            ...
            competitors: searchResults.slice(0, 10)
        });

    } catch (err) {
        console.error("Position Tracker Error:", err.message);
        // Fallback to a mock with a "simulated" rank if blocked
        res.json({
            keyword,
            domain,
            region,
            rank: Math.floor(Math.random() * 20) + 1, // Simulated for demo purposes if blocked
            totalResults: 10,
            competitors: [
            ...
            _note: "Request was rate-limited or blocked; showing simulated data."
        });
    }
});
...
app.post('/api/youtube/strategy', async (req, res) => {
    ...
    try {
        ...
        const prompt = `
            ...
            GOAL: Give the user UNIQUE, aggressive, and human-viral psychology-backed ideas that they haven't heard before. Avoid generic advice like "be consistent".
            ...
            {
                "monthlyPerformance": { "performanceVerdict": "A breakdown of their 'viral momentum' and why they are or aren't growing right now." },
                "contentStrategy": { 
                    "optimalTimeLimit": "Specific video length suggestion based on niche data.", 
                    "remixedIdeas": [{ 
                        "title": "A title that uses a high-CTR 'Curiosity Gap'.", 
                        "concept": "A unique video structure or 'Challenge' idea.", 
                        "thumbnailHook": "A visual description for the thumbnail.", 
                        "hookLogic": "The psychology behind why someone WILL click." 
                    }] 
                },
                "marketAnalysis": { "competitionLevel": "...", "marketGap": "A specific 'White Space' or topic no one in their niche is talking about." },
                "audienceDemand": { "topSearchTerms": ["..."], "currentTrend": "The next 3-month trend prediction for this niche." }
            }
        `;
