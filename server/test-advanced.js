...
async function test() {
    ...
    const stats = await apiReq('/channels/@mrbeast/stats?range=60&withRevenue=true');
    const averages = await apiReq('/channels/@mrbeast/averages');
    const split = await apiReq('/channels/@mrbeast/longsAndShorts');

    console.log('Channel Data:', !!channel);
    console.log('Stats Array Length:', stats?.length || 'NULL');
